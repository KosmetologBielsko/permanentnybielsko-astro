...
if (!(Test-Path $ImagePath)) {
  Write-Host "ERROR: Nie znaleziono obrazu:" -ForegroundColor Red
  Write-Host $ImagePath -ForegroundColor Yellow
  Write-Host "Najpierw wklej plik zakres-zabiegow-pmu-boguslawa-herda.webp do public/images." -ForegroundColor Yellow
  exit 1
}
...
if ($Text -notmatch "pmu-services-collage") {
  ...
  Write-Host "INFO: Kolaż już jest w pliku — nie dodaję drugi raz." -ForegroundColor Cyan
}
...
if ($Text -notmatch "V8 — kolaż Zakres zabiegów PMU") {
  ...
  Write-Host "INFO: CSS kolażu już jest w pliku — nie dodaję drugi raz." -ForegroundColor Cyan
}
